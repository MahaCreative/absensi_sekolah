...
export default function Index(props) {
    ...
    const { data: kelas } = props.kelas;
    const [params, setParams] = useState({ page: "", q: "", search: "" });
    const [modalForm, setModalForm] = useState(false);
    const [model, setModel] = useState();
    const reload = useCallback(
        debounce((query) => {
            router.get(route("admin.kelola-kelas"), pickBy({ ...query }), {
                preserveScroll: true,
                preserveState: true,
            });
        }, 500), // ⬅️ delay 500ms
    ...
    const deleteHandler = (id) => {
        router.delete(route("admin.delete-kelas", { id: id }), {
            onSuccess: () => {
                showAlert(
                    "success",
                    "Berhasil",
                    "Berhasil menghapus 1 data kelas pada database"
                );
            },
        });
    };
            ...
            <div className="my-3 py-2 px-4 bg-white rounded-md drop-shadow-md overflow-x-auto w-full">
                <div className="flex justify-between items-center py-3 px-3 w-full">
                    <button
                        ...
                        className="flex text-xs md:text-base leading-3 items-center  mr-2 gap-x-3 py-2 px-4 rounded-md bg-blue-500 hover:bg-blue-600 text-white"
                    >
                        <Add />
                        <p>Tambah Kelas</p>
                    </button>
                        ...
                        {kelas.length > 0 ? (
                            kelas.map((item, key) => (
                                ...
                                >
                                    ...
                                    <td className="py-3 px-4">
                                        {item.laki_count} Siswa
                                    </td>
                                    <td className="py-3 px-4">
                                        {item.perempuan_count} Siswa
                                    </td>
...
Index.layout = (page) => <Layouts children={page} title={"Kelola Kelas"} />;
